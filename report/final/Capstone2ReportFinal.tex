...
\section{Alternative Algorithms}
  ...
  The maps can be represented using a \textit{search space tree}, which has a dummy root. Each node represents a possible match between a vertex in graph $G$ of the pattern in the target graph $G'$.

  The path from the root to a node

  Bonnici's algorithm has a significant reduction in running time compared to VF2 (in its VFlib implementation). However, due to a--
  \includegraphics[scale=0.5]{images/bonnici_searchtree.png}
